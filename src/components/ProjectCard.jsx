...
const ProjectCard = ({ project, userID, handleDelete }) => {
    const { title, description, github_url, demo_url, image_url, tags } =
        project
    ...
    return (
        <div className='grid grid-cols-[5fr_2fr] bg-secondary-bg rounded-lg shadow-md overflow-hidden'>

            <div className='p-6 flex flex-col'>
                {/* Date and Tags */}
                <div className='flex justify-between items-center'>
                    <span className='font-light text-gray-600'>March 1, 2023</span>
                            ...
                            <div className='space-x-2'>
                                {tags.map((tag) => (
                                    <Link to={`/tags/${tag.name}`} key={tag.id}>
                                        <span className='inline-block border border-primary-accent rounded-full px-2 py-1 text-xs font-semibold text-primary-accent hover:bg-primary-accent hover:bg-opacity-10 transition-colors duration-250'>
                                            {tag.name}
                                        </span>
                                    </Link>
                                ))}
