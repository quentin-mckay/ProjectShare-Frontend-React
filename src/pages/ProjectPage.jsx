			...
			{project ? (
				...
				<section>
					...
					<div className="bg-white shadow rounded p-4 mx-auto mt-4 max-w-2xl">
						...
						{project.demo_url?.length !== 0 && <a href={project.demo_url} className="text-blue-500 underline">
							Demo
						</a>}
					</div>
					...
					(<div className="bg-white shadow rounded p-4 mx-auto mt-4 max-w-2xl">
						...
						<ul className="flex flex-wrap">
							{project.tags.map(tag => (
								<Link to={`/tags/${tag.name}`} key={tag.id} className="mr-2 mb-2">
					...
					{/* <h2 className="text-lg font-bold text-gray-800 mb-2">Comments</h2> */}
						<form className="mb-4" onSubmit={handleCommentSubmit}>
							...
							<div className="flex flex-col mb-4">
								<label className="mb-2 font-bold text-gray-800" htmlFor="comment">Comment</label>
								<textarea
									id="comment"
									name="comment"
									rows="3"
					...
					{comments.map(comment => (
						<div key={comment.id} className="border-t border-gray-200 pt-4">
							...
							<div>
								<h3 className="font-bold text-gray-800">{comment.user.username}</h3>
